...
void Shader::Reload()
{
	...
	spdlog::debug("Reloaded shader! ({}, {})", m_VertexShaderPath, m_FragmentShaderPath);
}
...
void Shader::LinkShaderProgram()
{
	...
	if (result == GL_FALSE)
	{
		...
		spdlog::error("Failed to link shader program! ({}, {})", m_VertexShaderPath, m_FragmentShaderPath);
...
GLuint Shader::CompileShader(std::string path, bool isVertex) const
{
	...
	if (result == GL_FALSE)
	{
		...
		spdlog::error("Failed to compile {} shader! ({})", isVertex ? "Vertex" : "Fragment", path);
		spdlog::error("{}", message);
...
std::string Shader::GetFileContent(std::string path) const
{
	...
	{
		spdlog::error("Couldn't find shader file: {}", path);
...
GLint Shader::GetUniformLocation(const std::string& name)
{
	...
	else
	{
		...
		if (location == -1)
		{
			spdlog::error("Couldn't find uniform: {} in shader program! ({}, {})", name, m_VertexShaderPath, m_FragmentShaderPath);
